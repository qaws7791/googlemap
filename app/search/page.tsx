...
export default function SearchPage() {
  ...
  const getSido = async () => {
    ...
    if(data) {
      const sidoData = data.map(sido=>{ return ( { value:sido.properties.ctprvn_cd,name:sido.properties.ctp_kor_nm})} )
      setSido(sidoData)
  ...
  const getSigoon = async () => {
    ...
    if(data) {
      const sigoonData = data.map(sigoon=>{ return ( { value:sigoon.properties.sig_cd,name:sigoon.properties.sig_kor_nm})} )
  ...
  const getDong = async () => {
    ...
    if(data) {
      const dongData = data.map(dong=>{ return ( { value:dong.properties.emd_cd,name:dong.properties.emd_kor_nm})} )
      setdong(dongData)
